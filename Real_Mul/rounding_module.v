module rounding_module #(
    ...
    input  wire                       res_sign,
    output wire [HIGH_PART_WIDTH-1:0] data_out,  // готовое поле мантиссы (без скрытого бита)
    output wire                       inexact,   // Флаг точности (1 - округление потребовалось)
    output wire                       overflow   // Флаг переполнения (перенос за пределы мантиссы)
);

    wire [HIGH_PART_WIDTH:0]  high_part = data_in[TOTAL_WIDTH-1:LOW_PART_WIDTH]; // включает скрытый бит
    wire [LOW_PART_WIDTH-1:0] low_part  = data_in[LOW_PART_WIDTH-1:0];

    wire low_part_is_zero = (low_part == 0); // Флаг на то, что округления не требуется

    // Биты для округления к четному
    wire round_bit  = high_part[0];
    wire guard_bit  = low_part[LOW_PART_WIDTH-1];
    wire sticky_bit = |low_part[LOW_PART_WIDTH-2:0];
    ...
    wire [HIGH_PART_WIDTH:0] rounded_full = overflow ? {1'b1, {HIGH_PART_WIDTH{1'b0}}} : (high_part + increment);

    assign data_out = rounded_full[HIGH_PART_WIDTH-1:0];
